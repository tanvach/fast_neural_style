...
class TransformerNet(torch.nn.Module):
    def __init__(self, alpha = 1.0, use_small_network=False, use_separable_conv=False, image_size=256):
        ...
        if use_small_network:
            ...
            self.in4 = torch.nn.BatchNorm2d(int(alpha * 32), affine=True)
            ...
            self.in5 = torch.nn.BatchNorm2d(int(alpha * 32), affine=True)
            ...
            self.in2 = torch.nn.BatchNorm2d(int(alpha * 64), affine=True)
            self.conv3 = ConvLayer(int(alpha * 64), int(alpha * 128), kernel_size=3, stride=2, use_separable_conv=use_separable_conv)
            self.in3 = torch.nn.BatchNorm2d(int(alpha * 128), affine=True)
            # Residual layers
            ...
            self.in4 = torch.nn.BatchNorm2d(int(alpha * 64), affine=True)
...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.in1 = torch.nn.BatchNorm2d(channels, affine=True)
        self.conv2 = ConvLayer(channels, channels, kernel_size=3, stride=1)
        self.in2 = torch.nn.BatchNorm2d(channels, affine=True)
        self.relu = torch.nn.ReLU()
